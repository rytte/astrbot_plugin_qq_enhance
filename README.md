...
- 跨群和跨好友默认关闭。启用后，AstrBot 管理员可以从群聊或私聊发起跨会话操作，并且目标必须确实存在于机器人群或好友列表。
- `qq_user_info.stranger` 是只读例外：AstrBot 管理员在私聊中查询非好友公开资料时无需开启跨好友操作；从群聊发起时仍需开启。
- `qq_group_request.list` 是只读例外：AstrBot 管理员在私聊中查询全部群申请或按 `group_id` 筛选时无需开启跨群操作；从群聊跨群查询时仍需开启。
