...
- 账号、好友关系、好友申请、群列表和其他全局能力只允许 AstrBot 管理员或 `permissions.admin_users` 中的用户。
- 跨群和跨好友默认关闭。启用后仍要求管理员在私聊发起、目标位于精确 ID 白名单中，并且目标确实存在于机器人群或好友列表。
...
确认默认 60 秒过期且只能使用一次。删除好友、踢人、管理员变更、退群、删除群文件、公告、精华、撤回、群名/头像修改、全员禁言、单次禁言不少于 3600 秒，以及所有跨会话写操作都强制确认。配置只能增加确认项，不能移除这些安全边界。
...
- `permissions`：管理员、群角色策略、跨会话开关与精确白名单。
- `confirmation`：确认有效期和额外确认 operation。
