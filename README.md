...
插件将 QQ 表情、语音转写和结构化组件整理成模型可读文本。语音优先使用 AstrBot 已有转写，未转写的语音可由 NapCat 补充识别；戳一戳和红包可按开关唤醒模型。撤回感知只更新已有上下文，不因撤回事件主动回复。
...
- **撤回感知**：支持私聊和群聊，定位并标记已有上下文中的原消息；消息映射保留 180 秒。标记不会改变已经发给模型的请求，无法匹配已有上下文时可能无法标记。
